...
```bash
# 1. 还原页面 + 调试面板（核心命令）
python /Users/mac/Desktop/AIProject/SKILLS/PageGhost/scripts/mount_env.py --import <snapshot.json路径>

# 2. 如果页面样式缺失，先爬取静态资源再还原
python /Users/mac/Desktop/AIProject/SKILLS/PageGhost/scripts/crawler.py --snapshot <snapshot.json路径> --output /tmp/pageghost_assets
python /Users/mac/Desktop/AIProject/SKILLS/PageGhost/scripts/mount_env.py --import <snapshot.json路径> --assets /tmp/pageghost_assets
...
python mount_env.py --import snapshot.json --port 9090
python mount_env.py --import snapshot.json --assets ./crawled_assets
python mount_env.py --import snapshot.json --no-open
```
...
- 命令：`python crawler.py --snapshot snapshot.json --output ./assets`
